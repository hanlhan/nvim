...
### 1 基础编辑器特性
#### 1.1 最基本的键位
**`h`** : 切换至 **`插入`** : 一种模式 (在原版 Vim 中与 `i` 键相同)
...
**_重要_**
  ...
  因为 `i` 键被改为了 `h` 键, 所有包含 `i` 键的命令都必须将 `i` 改为 `h` (比如 `ciw` 应被更正为 `chw`)
...
| 快捷键      | 行为                                   |
|-------------|----------------------------------------|
| `s` `i`     | 新建一个分屏并把它放置在当前窗口的上面 |
| `s` `k`     | 新建一个分屏并把它放置在当前窗口的下面 |
| `s` `j`     | 新建一个分屏并把它放置在当前窗口的左边 |
